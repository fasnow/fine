...
import {SaveProxy, SaveQueryOnEnter, SaveTimeout,} from "../../wailsjs/go/application/Application";
...
export const Setting: React.FC = () => {
    ...
    const ICPForm: React.FC = () => {
        ...
        return <Flex justify={"left"} vertical gap={10}>
            ...
            <Number labelWidth={200} label={"单查询时认证错误重试次数"} value={cfg.ICP.AuthErrorRetryNum1}
                    onChange={value => update({...cfg.ICP, AuthErrorRetryNum1: value as number} as config.ICP)}/>
            <Number labelWidth={200} label={"单查询时403错误误重试次数"} value={cfg.ICP.ForbiddenErrorRetryNum1}
                    onChange={value => update({...cfg.ICP, ForbiddenErrorRetryNum1: value as number} as config.ICP)}/>
            <Number labelWidth={200} label={"批量查询时认证错误重试次数"} value={cfg.ICP.AuthErrorRetryNum2}
                    onChange={value => update({...cfg.ICP, AuthErrorRetryNum2: value as number} as config.ICP)}/>
            <Number labelWidth={200} label={"批量查询403错误误重试次数"} value={cfg.ICP.ForbiddenErrorRetryNum2}
                    onChange={value => update({...cfg.ICP, ForbiddenErrorRetryNum2: value as number} as config.ICP)}/>
            <Number labelWidth={200} label={"批量查询协程数"} value={cfg.ICP.Concurrency}
                    onChange={value => update({...cfg.ICP, Concurrency: value as number} as config.ICP)}/>
            <Number labelWidth={200} label={"批量查询代理超时（ns）"} width={200} value={cfg.ICP.Timeout}
    ...
    return (
        <Flex vertical gap={10} style={{height: '100%', overflow: 'auto', padding: '10px', boxSizing: 'border-box'}}>
            <Divider style={{marginTop: "0px", marginBottom: "0px", fontSize: '14px'}}
                     orientation="left">全局设置</Divider>
            ...
            />
            <Password label={'天眼查'} placeholder={"auth_token"} labelWidth={80} width={400}
