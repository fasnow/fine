...
import {FolderOpenOutlined,FileZipOutlined,FileTextOutlined } from "@ant-design/icons";
...
import {
    ...
    ClearDecompiled,
    Decompile,
    GetAllMiniProgram, GetMatchedString,
    ...
    return (
        <div style={{
            ...
            >
                ...
                <Flex align={"center"} justify={"center"}><>建议先执行清空Applet目录以避免无法回溯是哪个小程序</></Flex>
                                                        ...
                                                        item.versions.map((i)=>{
                                                            return <Flex gap={5} key={`${item.appid}-${i.number}`}>
                                                                ...
                                                                <Space.Compact size={"small"} >
                                                                    <Tooltip title={"反编译"}>
                                                                        <Button icon={<DecompileIcon style={{fontSize:"12px"}}/>} onClick={()=> {
                                                                    ...
                                                                    <Tooltip title={"敏感信息"}>
                                                                        <Button
                                                                            ...
                                                                            onClick={
                                                                                async () => {
                                                                                    setSelect({appid:item.appid,version:i.number,nickname:item.nickname})
                            ...
                            <Allotment.Pane className={"right-top"} preferredSize={500} minSize={150}>
                                <Flex vertical gap={5} style={{height:"100%"}}>
                                    <Flex>
                                        {
                                            select?.appid && <Flex>
                                                <Tag bordered={false} color={"orange"}>{select?.appid} {select?.version}</Tag>
                                            </Flex>
                                        }
                                        {
