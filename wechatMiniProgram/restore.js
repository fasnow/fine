...
function catchZGroup(code, groupPreStr) {
    ...
    for (let preStr of groupPreStr) {
        ...
        let z = [];
        //__APP__ (function(z) {\n\t      var a = 11;
        //subPackge (function(z) {\n           var a = 11;
        let index = content.indexOf("(function(z) {\n\t      var a = 11;")
        if (index < 0){
            index = content.indexOf("(function(z) {\n           var a = 11;")
        }
...
function catchZ(code) {
    // code是美化过后，正则进行了修改
    let groupTest = code.match(/function gz\$gwx(\d*\_\d+)\(\)\s\{\s*if\s\(__WXML_GLOBAL__\.ops_cached\.\$gwx\d*\_\d+\)/g);
    if (groupTest !== null) {
        return catchZGroup(code, groupTest);
    }
    let z = [], vm = new VM({
        sandbox: {
            z: z,
