...
var DefaultRegex = []string{
	...
	"(?:[1-9]\\d{5}(?:(?:18|19|20)\\d{2})(?:(?:0[1-9]|1[0-2]))(?:(?:0[1-9]|[12]\\d|3[01]))\\d{3}[\\dXx]|[1-9]\\d{7}(?:(?:0[1-9]|1[0-2]))(?:(?:0[1-9]|[12]\\d|3[01]))\\d{3})", // 身份证
	"(?:\\+?86)?1[3-9]\\d{9}", // 手机号码
	"(?i)(post|get|delete|put|url)[:\\(]+[^\\S\r\n]*[\"']\\/(?:[\\w-]+\\/)*(?:[\\w-]+)\\/*\\??(?:[\\w=&-]+)?[\"']\\)*",
	"(?i)http(s)?://(?:[\\w-]+\\/)*(?:[\\w-]+)\\/*\\??(?:[\\w\\=&-]+)?[\"']\\)*",
	"(?i)\\/(?:[\\w-]+\\/)*[\\w-]+(.cgi|.php|.action|.jsp|.jspx|.asp|.aspx|.py|.rb|.html|.htm|.tpl|.do|.jsf)?\\/*\\??(?:[\\w=&-]+)?\\)*", // url
	"(?i)[\"']?app[_-]?(id|secret)[\"']?[^\\S\r\n]*[=:][^\\S\r\n]*[\"']?[\\w-]+[\"']?",
