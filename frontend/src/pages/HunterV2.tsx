...
const TabContent:React.FC<TabContentProps>=(props)=> {
    ...
    useEffect(() => {
        if (ref.current){
            setTableScrollHeight(window.innerHeight - ref.current?.getBoundingClientRect().height - 165)
    ...
    const handleMenuItemClick: MenuProps['onClick'] = (e) => {
        if (!props.newTab || !selectedRow || !selectedRow.item || !selectedRow.colKey) {
            return
        }
        switch (e.key) {
            case MenuItems.QueryTitle.key:
                props.newTab && props.newTab("web.title=" + selectedRow.item.web_title, checkedColKeys, queryOption)
                break
