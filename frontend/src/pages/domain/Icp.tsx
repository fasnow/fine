import { CloudDownloadOutlined, CopyOutlined, GlobalOutlined, LoadingOutlined, SearchOutlined, SyncOutlined } from '@ant-design/icons';
import { Button, Input, MenuProps, Modal, Pagination, Space, Spin, Table, Tabs, message } from 'antd';
import React, { ReactNode, useEffect, useRef, useState } from 'react';
import type { ColumnsType } from 'antd/es/table';
import { $icpRecordExport, $icpRecordQuery } from '../../http/api';
import { QUERY_FIRST, IcpItemType, MenuItemsKey, copy } from '../../type';
...
let selectedRow: { item: IcpItemType, rowIndex: number, colKey: string } = {
    item: undefined,
    rowIndex: 0,
    colKey: ''
}
...
const IcpContent: React.FC = () => {
    const defaultColumns: ColumnsType<IcpItemType> = [
        {
        ...
        },
        {
            title: '主备案号', dataIndex: "mainLicense", ellipsis: true, width: 200, onCell: (record, index) => {
                return {
                    ...
                    onClick: () => copyCell(record.mainLicense)
                }
            }
        },
        {
    ...
    const [columns, setColumns] = useState<ColumnsType<IcpItemType>>(defaultColumns)
    ...
    function handleResize() {
        setTableHeight(window.innerHeight - 30 - 46 - 17 - 24 - 17 - 20 - 5 - 24 - 30);
    }
    ...
    const handleClickImage = (event: React.MouseEvent<HTMLImageElement>) => {
        ...
        if (clickCount == captcha.maxClicks) {
            const config: AxiosRequestConfig = {
                params: {
                    pointJson: encryptPoints(positions, captcha.secretKey),
                },
            };
            req.get(`/query/icpRecord/checkImage`, config)
                .then(
                    resp => {
                        const result = resp.data
                        if (result.code != 200) {
                            errorNotification("ICP无法获取签名", result.message)
                            return
                        }
                        if (result.data.sign == "") {
                            setCaptcha(pre => ({
                                ...pre,
                                smallImage: result.data.smallImage,
                            }))
                            sleep(500)
                            fetchImage()
                        } else {
    ...
    const fetchImage = () => {
        ...
        req.get("/query/icpRecord/getImage")
            .then(
                resp => {
                    const result = resp.data
                    if (result.code != 200) {
                        errorNotification("ICP无法获取验证码", result.message)
                        return
                    }
                    setCaptcha(pre => ({
                        ...pre,
                        bigImage: result.data.bigImage,
                        smallImage: result.data.smallImage,
                        secretKey: result.data.secretKey,
                        maxClicks: result.data.wordCount,
                        loading: false,
                        refetching: false
                    }))
                }
            ...
            .catch(
                (e) => {
                    errorNotification("ICP无法获取验证码", e)
                    setCaptcha(pre => ({
                        ...pre,
                        loading: false
                    }))
    ...
    async function handleQuery(unitName: string, page: number, pageSize: number,) {
        ...
        const result = await $icpRecordQuery({ query: unitName, page: page, size: pageSize })
        if (result.error) {
            errorNotification("ICP查询出错", result.error)
            setLoading(false)
            return
        }
        setCurrentPage(page)
    ...
    const isSignExpired = async () => {
        try {
            const resp = await req.get("/query/icpRecord/isSignExpired")
            const result = resp.data
            return result.data
        } catch (error) {
    ...
    async function exportData() {
        ...
        setIsExporting(true)
        const result = await $icpRecordExport(id)
        if (result.error) {
            setIsExporting(false)
            errorNotification("导出结果", result.error)
            return
        }
        setIsExporting(false)
    ...
    const getMergeColumns = (): ColumnsType<IcpItemType> => {
        return columns.map((col, index) => ({
            ...col,
            onHeaderCell: (column: ColumnsType<IcpItemType>[number]) => ({
                width: column.width,
        ...
        switch (key) {
            case MenuItemsKey.OpenDomain:
                if (selectedRow.item?.domain) {
                    _openDefaultBrowser("http://" + selectedRow.item?.domain)
                }
                break
            case MenuItemsKey.CopyCell:
                {
                    const item = selectedRow.item as IcpItemType
                    for (const key in item) {
                        if (Object.prototype.hasOwnProperty.call(item, key) && key === selectedRow.colKey) {
                            const value = item[key as keyof IcpItemType]
                            copy(String(value))
                        }
                    }
                }
            ...
            case MenuItemsKey.CopyCol:
                {
                    const colValues = dataCache[currentPage].map(item => {
                        for (const key in item) {
                            if (Object.prototype.hasOwnProperty.call(item, key) && key === selectedRow.colKey) {
                                return String(item[key as keyof IcpItemType]);
                            }
                        }
                        return ""
                    })
