...
import { $zoneSiteQuery, $zoneSiteQueryExport, $zoneDomainQuery, $zoneApkQuery, $zoneEmailQuery, $zoneEmailQueryExport, $zoneMemberQueryExport, $zoneMemberQuery, $zoneCodeQuery, $zoneCodeQueryExport, $zoneDarknetQuery, $zoneDarknetQueryExport, $save0zoneConf, $zoneQueryExportStatus } from '../../http/api';
import { QUERY_FIRST, IcpItemType, MsgOfWechatType, ZoneSiteItemType, MenuItemsKey, ZoneDomainItemType, ZoneApkItemType, MsgOfMiniProgramType, MsgOfApkType, ZoneEmailItemType, ZoneMemberItemType, ZoneCodeItemType, ZoneCodeDetailParsingType, ZoneDarknetItemType, copy } from '../../type';
...
import { CodeData, DarknetData } from '../../testdata/testdata';
import NotFound from '../Notfound';
import { ResizeCallbackData } from 'react-resizable';
import ResizableTitle from '../../component/ResizableTitle';
import { _openDefaultBrowser } from '../../electron/electronApi';
import { useDispatch, useSelector } from 'react-redux';
import { RootState, setHasNewLogItem, setZoneAuth } from '../../store/store';
...
type dataCacheType = {
    [key: number]: IcpItemType[];
}

const columns: ColumnsType<IcpItemType> = [
    { title: '序号', dataIndex: "index", width: 50, ellipsis: true, fixed: "left", },
    { title: '名称', dataIndex: "unitName", width: 250, ellipsis: true, fixed: "left", },
    { title: '域名', dataIndex: "domain", width: 200, ellipsis: true, fixed: "left", },
    { title: '备案号', dataIndex: "serviceLicence", width: 200, ellipsis: true, },
    { title: '单位性质', dataIndex: "natureName", width: 100, ellipsis: true, },
    { title: '审核日期', dataIndex: "updateRecordTime", width: 200, ellipsis: true, },
    { title: '主备案号', dataIndex: "mainLicense", width: 200, ellipsis: true, },
    { title: '备案法人', dataIndex: "leaderName", width: 200, ellipsis: true, },
    { title: '域名ID', dataIndex: "domainId", width: 200, ellipsis: true, },
    { title: '限制访问', dataIndex: "limitAccess", width: 200, ellipsis: true, },
    { title: 'mainId', dataIndex: "mainId", width: 200, },
    { title: 'serviceId', dataIndex: "serviceId", width: 200, ellipsis: true, },
];
...
const TabContent: React.FC = () => {
    ...
    async function query() {
        ...
        if (input.includes("&&") || input.includes("||") || input.includes("(") || input.includes(")") || input.includes("=") || input.includes("==") || input.includes("===")
            ...
            ref[activeKey].current.query(tmpInput,1,40);
            return
        }
        ref.site.current.query(tmpInput,1,40);
        ref.domain.current.query(tmpInput,1,40);
        ref.apk.current.query(tmpInput,1,40);
        ref.member.current.query(tmpInput,1,40);
        ref.email.current.query(tmpInput,1,40);
...
const SiteTabContent = forwardRef((props, ref) => {
    ...
    const selectedRow = useRef<{ item: ZoneSiteItemType, rowIndex: number, colKey: string }>()
    type dataCacheType = {
        [key: number]: ZoneSiteItemType[];
    }
    const [columns, setColumns] = useState<ColumnsType<ZoneSiteItemType>>([
        {
        ...
        {
            title: '端口', dataIndex: "port", ellipsis: true, width: 50, sorter: ((a, b) => localeCompare(a.port, b.port)), onCell: (record, index) => {
                return {
        ...
        {
            title: 'CMS', dataIndex: "cms", ellipsis: true, width: 60, sorter: ((a, b) => localeCompare(a.cms, b.cms)), onCell: (record, index) => {
                return {
        ...
        {
            title: '城市', dataIndex: "city", ellipsis: true, width: 60, sorter: ((a, b) => localeCompare(a.city, b.city)), onCell: (record, index) => {
                return {
        ...
        {
            title: 'Operator', dataIndex: "operator", ellipsis: true, width: 60, sorter: ((a, b) => localeCompare(a.operator, b.operator)), onCell: (record, index) => {
                return {
        ...
        {
            title: 'CDN', dataIndex: "is_cdn", ellipsis: true, width: 50, sorter: ((a, b) => localeCompare(a.is_cdn, b.is_cdn)), render: (text) => text == 0 ? "否" : "是", onCell: (record, index) => {
                return {
        ...
        {
            title: '证书', dataIndex: "ssl_certificate", ellipsis: true, width: 50, sorter: ((a, b) => localeCompare(a.ssl_certificate, b.ssl_certificate)), onCell: (record, index) => {
                return {
        ...
        {
            title: '服务', dataIndex: "service", ellipsis: true, width: 50, sorter: ((a, b) => localeCompare(a.service, b.service)), onCell: (record, index) => {
                return {
    ...
    useImperativeHandle(ref, () => ({
        query: (input: string,page:number,pageSize:number)=>handleQuery(input,page,pageSize),
    }));

    const handleResize = () => {
        setTableHeight(window.innerHeight - 30 - 46 - 17 - 24 - 17 - 40 - 5 - 30);
    }
    ...
    const handleQuery = async (input: string,page:number,pageSize:number) => {
        const tmpInput = input.trim()
        if (tmpInput === "") {
            return
        }
        ...
        const result = await $zoneSiteQuery({ query: tmpInput, page: page, size: pageSize })
        if (result.error) {
            errorNotification("0.zone查询出错", result.error)
            setDataCache({})
            setLoading(false)
            return
        }
    ...
    const handlePaginationChange = async (newPage: number, newSize: number) => {
        ...
        if (newPage != currentPage && newSize==currentPageSize) {
            ...
            //没有缓存则获取
            const result = await $zoneSiteQuery({ page: newPage, size: currentPageSize, query: inputCache })
            if (result.error) {
                errorNotification("0.zone查询出错", result.error)
                setLoading(false)
                return
        ...
        switch (key) {
            case MenuItemsKey.OpenUrl:
                if (selectedRow.current.item.url) {
                    _openDefaultBrowser(selectedRow.current.item.url)
                }
                break
            case MenuItemsKey.CopyCell:
                {
                    const item = selectedRow.current.item as ZoneSiteItemType
                    for (const key in item) {
                        if (Object.prototype.hasOwnProperty.call(item, key) && key === selectedRow.current.colKey) {
                            copy(item[key as keyof ZoneSiteItemType])
                        }
                    }
                }
                break
            ...
            case MenuItemsKey.CopyCol:
                {
                    const values = dataCache[currentPage].map(item => {
                        for (const key in item) {
                            if (Object.prototype.hasOwnProperty.call(item, key) && key === selectedRow.current.colKey) {
                                return item[key as keyof ZoneSiteItemType]
                            }
                        }
                        return ""
                    })
    ...
    const getMergeColumns = (): ColumnsType<ZoneSiteItemType> => {
        return columns.map((col, index) => ({
            ...col,
            onHeaderCell: (column: ColumnsType<ZoneSiteItemType>[number]) => ({
                width: column.width,
...
const DomainTabContent = forwardRef((props, ref) => {
    const selectedRow = useRef<{ item: ZoneDomainItemType, rowIndex: number, colKey: string }>()
    type dataCacheType = {
        [key: number]: ZoneDomainItemType[];
    }
    const [columns, setColumns] = useState<ColumnsType<ZoneDomainItemType>>([
        {
    ...
    const handleQuery = async (input: string,page:number,pageSize:number) => {
        ...
        //不能使用inputCache，setInputCache(tmpInput)为异步更新，此时inputCache还没有更新
        const result = await $zoneDomainQuery({ query: tmpInput, page: page, size: pageSize })
        if (result.error) {
            errorNotification("0.zone查询出错", result.error)
            setDataCache({})
            setLoading(false)
    ...
    const handlePaginationChange = async (newPage: number, newSize: number) => {
        ...
        if (newPage != currentPage && newSize==currentPageSize) {
            ...
            //没有缓存则获取
            const result = await $zoneDomainQuery({ page: newPage, size: currentPageSize, query: inputCache })
            if (result.error) {
                errorNotification("0.zone查询出错", result.error)
                setLoading(false)
                return
            ...
            case MenuItemsKey.OpenDomain:
                {
                    ...
                    if (url) {
                        _openDefaultBrowser(url.startsWith("http") ? url : "http://" + url)
                    }
                    break
                }
            case MenuItemsKey.CopyCell:
                {
                    const item = selectedRow.current.item as ZoneDomainItemType
                    for (const key in item) {
                        if (Object.prototype.hasOwnProperty.call(item, key) && key === selectedRow.current.colKey) {
                            copy(item[key as keyof ZoneDomainItemType])
                        }
                    }
                }
                break
            ...
            case MenuItemsKey.CopyCol:
                {
                    const values = dataCache[currentPage].map(item => {
                        for (const key in item) {
                            if (Object.prototype.hasOwnProperty.call(item, key) && key === selectedRow.current.colKey) {
                                return item[key as keyof ZoneDomainItemType]
                            }
                        }
                        return ""
                    })
    ...
    const getMergeColumns = (): ColumnsType<ZoneDomainItemType> => {
        return columns.map((col, index) => ({
            ...col,
            onHeaderCell: (column: ColumnsType<ZoneDomainItemType>[number]) => ({
                width: column.width,
                onResize: handleHeaderResize(index) as React.ReactEventHandler<any>,
            }),
        }));
...
const ApkTabContent = forwardRef((props, ref) => {
    type dataCacheType = {
        [key: number]: ZoneApkItemType[];
    }
    const [columns, setColumns] = useState<ColumnsType<ZoneApkItemType>>([
    ...
    const [input, setInput] = useState<string>("")
    const [inputCache, setInputCache] = useState<string>("")
    const [dataCache, setDataCache] = useState<dataCacheType>({ 1: [] })
    const [total, setTotal] = useState<number>(0)
    const [currentPage, setCurrentPage] = useState<number>(1)
    const [currentPageSize, setCurrentSize] = useState<number>(pageSizeOptions[0])
    const [maxPage, setMaxPage] = useState<number>(0)
    ...
    const handleQuery = async (input: string,page:number,pageSize:number) => {
        ...
        //不能使用inputCache，setInputCache(tmpInput)为异步更新，此时inputCache还没有更新
        const result = await $zoneApkQuery({ query: tmpInput, page: page, size: pageSize })
        if (result.error) {
            errorNotification("0.zone查询出错", result.error)
            setDataCache({})
            setLoading(false)
    ...
    const handlePaginationChange = async (newPage: number, newSize: number) => {
        ...
        if (newPage != currentPage&& newSize==currentPageSize) {
            ...
            //没有缓存则获取
            const result = await $zoneApkQuery({ page: newPage, size: currentPageSize, query: inputCache })
            if (result.error) {
                errorNotification("0.zone查询出错", result.error)
                setLoading(false)
                return
    ...
    const getMergeColumns = (): ColumnsType<ZoneApkItemType> => {
        return columns.map((col, index) => ({
            ...col,
            onHeaderCell: (column: ColumnsType<ZoneApkItemType>[number]) => ({
                width: column.width,
                onResize: handleHeaderResize(index) as React.ReactEventHandler<any>,
            }),
        }));
...
const EmailTabContent = forwardRef((props, ref) => {
    type dataCacheType = {
        [key: number]: ZoneEmailItemType[];
    }
    const selectedRow = useRef<{ item: ZoneEmailItemType, rowIndex: number, colKey: string }>()
    const [columns, setColumns] = useState<ColumnsType<ZoneEmailItemType>>([
        {
            title: '序号', dataIndex: "index", ellipsis: true, width: 50, onCell: (record, index) => {
                return {
                    onContextMenu: () => { selectedRow.current = { item: record, rowIndex: index, colKey: "index", }; }
    ...
    const handleQuery = async (input: string,page:number,pageSize:number) => {
        ...
        //不能使用inputCache，setInputCache(tmpInput)为异步更新，此时inputCache还没有更新
        const result = await $zoneEmailQuery({ query: tmpInput, page: page, size: pageSize })
        if (result.error) {
            errorNotification("0.zone查询出错", result.error)
            setDataCache({})
            setLoading(false)
    ...
    const handlePaginationChange = async (newPage: number, newSize: number) => {
        ...
        if (newPage != currentPage&& newSize==currentPageSize) {
            ...
            //没有缓存则获取
            const result = await $zoneEmailQuery({ page: newPage, size: currentPageSize, query: inputCache })
            if (result.error) {
                errorNotification("0.zone查询出错", result.error)
                setLoading(false)
                return
    ...
    const exportData = async (page: number) => {
        ...
        setIsExporting(true)
        const error = await $zoneEmailQueryExport(id, page)
        if (error) {
            errorNotification("导出结果", error)
            setIsExporting(false)
            return
    ...
    const getMergeColumns = (): ColumnsType<ZoneEmailItemType> => {
        return columns.map((col, index) => ({
            ...col,
            onHeaderCell: (column: ColumnsType<ZoneEmailItemType>[number]) => ({
                width: column.width,
                onResize: handleHeaderResize(index) as React.ReactEventHandler<any>,
            }),
        }));
            ...
            case MenuItemsKey.CopyCol:
                {
                    const values = dataCache[currentPage].map(item => {
                        for (const key in item) {
                            if (Object.prototype.hasOwnProperty.call(item, key) && key === selectedRow.current.colKey) {
                                return item[key as keyof ZoneEmailItemType]
                            }
                        }
                        return ""
                    })
...
const MemberTabContent = forwardRef((props, ref) => {
    type dataCacheType = {
        [key: number]: ZoneMemberItemType[];
    }
    const [columns, setColumns] = useState<ColumnsType<ZoneMemberItemType>>([
        {
            title: '序号', dataIndex: "index", ellipsis: true, width: 50, onCell: (record, index) => {
                return {
                    onContextMenu: () => { selectedRow.current = { item: record, rowIndex: index, colKey: "index", }; }
    ...
    const [loading, setLoading] = useState(false)
    const dispatch = useDispatch()
    const selectedRow = useRef<{ item: ZoneMemberItemType, rowIndex: number, colKey: string }>()
    const [isExporting, setIsExporting] = useState<boolean>(false)
    const [messageApi, contextHolder] = message.useMessage();
    const copyCell = (value: string | number | boolean) => {
        if (!value) {
    ...
    const handleQuery = async (input: string,page:number,pageSize:number) => {
        ...
        //不能使用inputCache，setInputCache(tmpInput)为异步更新，此时inputCache还没有更新
        const result = await $zoneMemberQuery({ query: tmpInput, page: page, size: pageSize })
        if (result.error) {
            errorNotification("0.zone查询出错", result.error)
            setDataCache({})
            setLoading(false)
    ...
    const handlePaginationChange = async (newPage: number, newSize: number) => {
        ...
        if (newPage != currentPage&& newSize==currentPageSize) {
            ...
            //没有缓存则获取
            const result = await $zoneMemberQuery({ page: newPage, size: currentPageSize, query: inputCache })
            if (result.error) {
                errorNotification("0.zone查询出错", result.error)
                setLoading(false)
                return
    ...
    const exportData = async (page: number) => {
        ...
        setIsExporting(true)
        const error = await $zoneMemberQueryExport(id, page)
        if (error) {
            errorNotification("导出结果", error)
            setIsExporting(false)
            return
    ...
    const handleMenuItemClick = (key: MenuItemsKey) => {
        switch (key) {
            case MenuItemsKey.CopyCell:
                {
                    const item = selectedRow.current.item as ZoneMemberItemType
                    for (const key in item) {
                        if (Object.prototype.hasOwnProperty.call(item, key) && key === selectedRow.current.colKey) {
                            copy(item[key as keyof ZoneMemberItemType])
                        }
                    }
                }
                break
            ...
            case MenuItemsKey.CopyCol:
                {
                    const values = dataCache[currentPage].map(item => {
                        for (const key in item) {
                            if (Object.prototype.hasOwnProperty.call(item, key) && key === selectedRow.current.colKey) {
                                return item[key as keyof ZoneMemberItemType]
                            }
                        }
                        return ""
                    })
...
const CodeTabContent = forwardRef((props, ref) => {
    ...
    const handleResize = () => {
        setTableHeight(window.innerHeight - 30 - 46 - 17 - 24 - 17 - 40 - 5 - 5);
    }
    ...
    const handleQuery = async (input: string,page:number,pageSize:number) => {
        ...
        //不能使用inputCache，setInputCache(tmpInput)为异步更新，此时inputCache还没有更新
        const result = await $zoneCodeQuery({ query: tmpInput, page: page, size: pageSize })
        if (result.error) {
            errorNotification("0.zone查询出错", result.error)
            setDataCache({})
            setLoading(false)
    ...
    const handlePaginationChange = async (newPage: number, newSize: number) => {
        ...
        if (newPage != currentPage&& newSize==currentPageSize) {
            ...
            //没有缓存则获取
            const result = await $zoneCodeQuery({ page: newPage, size: currentPageSize, query: inputCache })
            if (result.error) {
                errorNotification("0.zone查询出错", result.error)
                setLoading(false)
                return
    ...
    const exportData = async (page: number) => {
        ...
        setIsExporting(true)
        const error = await $zoneCodeQueryExport(id, page)
        if (error) {
            errorNotification("导出结果", error)
            setIsExporting(false)
            return
        ...
        <>
            <Spin
                ...
                <div style={{
                    ...
                    {
                        !dataCache[currentPage] || dataCache[currentPage]?.length == 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} /> :
                            <AdList
                                split={false}
                                        ...
                                        >
                                            <Space direction='vertical' style={{ width: "100%" }} >
                                                <Space wrap style={{ width: "100%" }}>
                                                    ...
                                                    {
                                                        item.related_company?.length > 0 && <Tag bordered={false} color="red">关联企业: {item.related_company.join("、")}</Tag>
                                                    }
                                                    {
                                                        ops?.length > 0 && (
                                                            <>
...
const DarknetTabContent = forwardRef((props, ref) => {
    ...
    const handleQuery = async (input: string,page:number,pageSize:number) => {
        ...
        //不能使用inputCache，setInputCache(tmpInput)为异步更新，此时inputCache还没有更新
        const result = await $zoneDarknetQuery({ query: tmpInput, page: page, size: pageSize })
        if (result.error) {
            errorNotification("0.zone查询出错", result.error)
            setDataCache({})
            setLoading(false)
    ...
    const handlePaginationChange = async (newPage: number, newSize: number) => {
        ...
        if (newPage != currentPage&& newSize==currentPageSize) {
            ...
            //没有缓存则获取
            const result = await $zoneDarknetQuery({ page: newPage, size: currentPageSize, query: inputCache })
            if (result.error) {
                errorNotification("0.zone查询出错", result.error)
                setLoading(false)
                return
