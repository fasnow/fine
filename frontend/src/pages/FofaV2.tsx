...
interface TabContentProps {
    ...
    input?: string,
    full?: boolean,//全量搜索
    newTab?:(input:string,checkedColKeys:string[])=>void
}
...
const TabContent:React.FC<TabContentProps> =(props)=> {
    ...
    useEffect(() => {
        if (ref.current){
            setTableScrollHeight(window.innerHeight - ref.current?.getBoundingClientRect().height - 170)
        ...
        setColumns(tmp)
        if (input) {
            handleNewQuery(input, tmp, currentPageSize)
        }
    }, [])
    ...
    const handleMenuItemClick: MenuProps['onClick'] = (e) => {
        ...
        switch (e.key) {
            case MenuItems.QueryIpCidr.key:
                selectedRow?.item?.ip && props.newTab && props.newTab("ip=" + selectedRow?.item?.ip + "/24", checkedColKeys)
                break
            case MenuItems.QueryIP.key:
                selectedRow?.item?.ip && props.newTab && props.newTab("ip=" + selectedRow?.item?.ip, checkedColKeys)
                break
            case MenuItems.QueryTitle.key:
                selectedRow?.item?.title && props.newTab && props.newTab("title=" + selectedRow?.item?.title, checkedColKeys)
                break
            case MenuItems.OpenUrl.key:
                selectedRow?.item?.link && props.newTab && BrowserOpenURL(selectedRow.item.link)
                break
    ...
    const handleNewQuery = async (query: string, columns: ColumnsType<PageDataType>, pageSize: number) => {
        ...
        Query(0, t , 1, pageSize, tmp.join(","), full).then(
            (result) => {
                updateRestToken()
                let index = 0
    ...
    const handlePaginationChange = async (newPage: number, newPageSize: number) => {
        ...
        if (newPage !== currentPage && newPageSize === currentPageSize) {
            ...
            Query(pageID, inputCache, newPage, currentPageSize, tmp.join(","),full).then(
                result => {
                    updateRestToken()
        ...
        if (newPageSize !== currentPageSize) {
            ...
            handleNewQuery(input, columns, newPageSize)
        }
    }
    ...
    return <Flex vertical gap={5}>
        <Flex ref={ref} justify={'center'} align={"center"} gap={5}>
            <Candidate
                ...
                allowClear
                value={input}
                onSearch={(value)=>handleNewQuery(value, columns, currentPageSize)}
                onPressEnter={(value)=> {
                    if(!allowEnterPress)return
                    handleNewQuery(value, columns, currentPageSize)
                }}
                items={[
                    {
                        // onSelectItem:(item)=>{
...
const Fofa = ()=> {
    ...
    useEffect(()=>{
        ...
        setItems([{
            ...
            key: key,
            children: <TabContent checkedColKeys={defaultCheckedColKeys} newTab={addTab}/>,
        }])
        setActiveKey(key)
    },[])
    ...
    const addTab = (input: string, checkedColKeys: string[]) => {
