...
type RSDQueryResult struct {
	Data  []*RealtimeServiceItem `json:"items"`
	Page  int                    `json:"page"`
	Size  int                    `json:"size"`
...
func (r *realtimeData) Service(req *GetRealtimeDataReq) (*RSDQueryResult, error) {
	...
	request, err := http.NewRequest("POST", sdk.QuakeRealTimeServiceDataApiUrl, postData)
	if err != nil {
		return nil, err
	}
	...
	var resultItems []*RealtimeServiceItem
...
type RHDQueryResult struct {
	Data  []*RHDQueryResultItem `json:"items"`
	Page  int                   `json:"page"`
	Size  int                   `json:"size"`
	Total int64                 `json:"total"`
}
...
func (r *realtimeData) Host(req *GetRealtimeDataReq) (*RHDQueryResult, error) {
	...
	var resultItems []*RHDQueryResultItem
	err = json.Unmarshal(byteData, &resultItems)
	if err != nil {
		return nil, err
	}
...
func (q *Quake) parser(request *http.Request) ([]byte, int, int, int64, error) {
	...
	if err != nil {
		return nil, 0, 0, 0, sdk.UnexpectedStructureError
	}
...
func (f *faviconSimilarityData) Get(faviconHash string, similar float64, size int, ignoreCache bool, startTime string, endTime string) ([]*map[string]any, error) {
	...
	req, _ = http.NewRequest("POST", sdk.QuakeFaviconSimilarityDataApiUrl, reqBody)
	...
	if err != nil && err.Error() == "Key path not found" {
		return nil, errors.New("quake query failed:" + sdk.UnexpectedResponse)
	}
...
func (f *filterField) parser(fieldType QueryType) ([]*string, error) {
	...
	switch fieldType {
	case Service:
		req, _ = http.NewRequest("GET", sdk.QuakeServiceDataFilterFieldApiUrl, nil)
		break
	case AggregationService:
		req, _ = http.NewRequest("GET", sdk.QuakeAggregationServiceDataApiUrl, nil)
		break
	case Host:
		req, _ = http.NewRequest("GET", sdk.QuakeHostDataFilterFieldApiUrl, nil)
		break
	case AggregationHost:
		req, _ = http.NewRequest("GET", sdk.QuakeAggregationHostDataApiUrl, nil)
		break
	default:
		return nil, sdk.UnexpectedFieldTypeError
	}
...
func (q *Quake) User() (*User, error) {
	req, _ := http.NewRequest("GET", sdk.QuakeUserApiUrl, nil)
	//req.Header.Set("Content-Type", "application/json")
	body, err := q.send(req)
	if err != nil {
		return nil, err
	...
	if err != nil && err.Error() == sdk.WrongJsonFormat {
		return nil, errors.New(sdk.QuakeUnexpectedJsonResponse)
	} else if err != nil {
		return nil, errors.New(sdk.QuakeUnexpectedJsonResponseOfUser)
	}
	return &tmpResponse.User, nil
}

func Export(items []*RealtimeServiceItem, filename string) error {
	var headers = []any{"ID", "IP", "域名", "主机", "协议", "响应码", "网页标题", "组件", "网站路径", "中间件", "证书", "操作系统", "运营商", "地理位置", "场景", "自治域", "自治域编号", "更新时间"}
	var data = [][]any{headers}
	for i, item := range items {
		var component []string
		for _, comp := range item.Components {
			component = append(component, fmt.Sprintf("%s %s", comp.ProductNameEn, comp.Version))
		}
...
func (q *Quake) send(request *http.Request) ([]byte, error) {
	...
	if string(body) == sdk.QuakeUnauthorized {
		return nil, errors.New(sdk.QuakeInvalidKey)
	}
	return body, nil
}
