...
import { $ip138Domain2HistoryIps, $ip138Domain2Ips, $ip138Ip2Domains } from '../../http/api';
...
type ResultType1 = { domain: string, current?: { ip: string, locationOrDate: string }[], history?: { ip: string, locationOrDate: string }[], msg?: string }
type ResultType2 = { ip: string, current?: { domain: string, date: string }[], msg?: string }
...
const IP138Content: React.FC = () => {
    ...
    const [ips, setIps] = useState<ResultType1[]>()
    const [domains, setDomains] = useState<ResultType2[]>()
    ...
    const query = async (mode: ModeType, targetList: string[]) => {
        ...
        for (let i = 0; i < targetList.length; i++) {
            ...
            const target = targetList[i]
            if (mode == "ip2domains") {
                const result = await $ip138Ip2Domains(target)
                if (result.error) {
                    setDomains((pre) => [...pre, ...[{ ip: target, msg: result.error.toString(), current: [] }]])
                    await sleep(interval.current)
                    continue
                }
                setDomains((pre) => [...pre, ...[{ ip: target, current: result.items ? result.items : result.items ? result.items : [] }]])
                continue
            } else if (mode == "domain2ips") {
                const result = await $ip138Domain2Ips(target)
                if (result.error) {
                    setIps((pre) => [...pre, ...[{ domain: target, msg: result.error.toString(), current: [] }]])
                    continue
                } else if (result.message) {
                    setIps((pre) => [...pre, ...[{ domain: target, msg: result.message, current: [] }]])
                    continue
                }
                const result2 = await $ip138Domain2HistoryIps(target)
                if (result2.error) {
                    setIps((pre) => [...pre, ...[{ domain: target, current: result.items }]])
                    continue
                }
                setIps((pre) => [...pre, ...[{ domain: target, current: [...result.items, ...result2.items] }]])
                continue
            }
        }
    ...
    return (<div >
        ...
        <div style={{ display: 'flex', justifyContent: "center", width: '100%', color: "red", height: "30px", margin: "3px" }}>
            {
