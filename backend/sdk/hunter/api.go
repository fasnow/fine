...
func (h *Hunter) Get(req *GetDataReq) (*Result, error) {
	...
	url := fmt.Sprintf("%v?%s", sdk.HunterApiUrl, req.req.QueryParams.Encode())
	...
	result.Data = tmpResponse.Data.Arr
	return &result, nil
}

func Export(items []*Item, filename string) error {
	if len(items) == 0 {
		return nil
	}
