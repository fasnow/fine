...
var (
	...
	refreshTokenUrl string // refreshTokenUrl 刷新token的url，刷新token时用的是获取token时中的"refresh"值
...
func (i *ICP) Query(unitName string) (*Result, error) {
	...
	if unitName == "" {
		return nil, sdk.UnexpectedQueryStatementError
	}
	if i.page <= 0 {
		i.page = 1
	}
...
func (i *ICP) GetImage() (*Image, error) {
