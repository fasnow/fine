...
func (i *IP138) getToken() (string, error) {
	var token string
	url1 := fmt.Sprintf("%s%s/", sdk.Ip138BaseURL, i.domain)
...
func (d *domain) GetHistoryIP(domain string) ([]*IPItem, error) {
	url1 := fmt.Sprintf("%s%s", sdk.Ip138BaseURL, domain)
	request, err := http.NewRequest("GET", url1, nil)
...
func (d *domain) read() (map[string]string, error) {
	...
	url1 := fmt.Sprintf("%s?%s", sdk.Ip138BaseReadURL, params.Encode())
	request, err := http.NewRequest("GET", url1, nil)
	// User-Agent必须为空,不然可能会出现302,read和write的User-Agent必须一致
	request.Header.Set("User-Agent", "")
	if err != nil {
...
func (d *domain) write() (int, error) {
	...
	url1 := fmt.Sprintf("%s?%s", sdk.Ip138BaseWriteURL, params.Encode())
	request, err := http.NewRequest("GET", url1, nil)
	if err != nil {
		return -1, err
	}
...
func (d *domain) getLocation(ip, sign string) (string, error) {
	...
	url1 := fmt.Sprintf("%s?%s", sdk.Ip138BaseLocateURL, params.Encode())
	request, err := http.NewRequest("GET", url1, nil)
...
func (i *ip) GetCurrentDomain(ip string) ([]*DomainItem, error) {
	url1 := fmt.Sprintf("%s%s/", sdk.Ip138BaseURL, ip)
	request, err := http.NewRequest("GET", url1, nil)
	// User-Agent必须为空,不然可能会出现302
	request.Header.Set("User-Agent", "")
	if err != nil {
